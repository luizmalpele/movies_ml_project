...
def MovieWizard():
    ...
    if submit_button:
        ...
        movies_data_merged_genres = get_column_dummies_from_list(movies_data_filtered, column_name='genres',
                                                                 merge_dummies=True)
        ...
        # -------------- images
        matches = get_posters(movie_posters, recommendations_df)
        cols = st.columns(len(matches))
        # with st.container():
        for i in range(len(cols)):
            with cols[i]:
                st.text(matches[i][1])
                st.image(f"../data/imgs/{matches[i][0]}.jpg")

        st.markdown("<h2 style='text-align: center; color: black;'>t-SNE 2-Dimensional Plane Visualization</h2>",
                    unsafe_allow_html=True)
...
def Explore():
    st.markdown("<h1 style='text-align: center; color: black;'>Explore Statistics behind your Favourite Films</h1>",
                unsafe_allow_html=True)
    hy.info(
        'The DataFrame is interactive and allows you to Sort, Filter, and apply various Exploratory function to the data!')
    AgGrid(movies_data)
...
def Statistics():
    ...
    st.markdown(
        ...
        unsafe_allow_html=True)
    st.markdown("<h2 style='text-align: center; color: black;'>Critics\'s scores Exploration</h2>",
                unsafe_allow_html=True)
    ...
    with st.form(key='column_list_form'):
        ...
        arg_column_list = st.selectbox('Select a colume to Visualize', ['genres', 'language', 'directors', 'country'],
                                       key=1)
